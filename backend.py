...
# GitHub config (must set in environment securely)
GITHUB_OWNER = os.environ.get("GITHUB_OWNER")      # e.g. 'babasiker826'
GITHUB_REPO = os.environ.get("GITHUB_REPO")        # e.g. 'nabi-hosting-files'
# Şifreli token (Base64 ile encode edilmiş)
ENCRYPTED_GITHUB_TOKEN = os.environ.get("GITHUB_TOKEN")  # Base64 encoded token
...
def decrypt_token(encrypted_token):
    """Base64 ile şifrelenmiş token'ı çöz"""
    ...
    try:
        ...
        try:
            # Eğer zaten plain text ise direkt kullan
            if encrypted_token.startswith('ghp_'):
...
app.secret_key = os.environ.get("SECRET_KEY", "nabi-hosting-secret-please-change")
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get("DATABASE_URL", "sqlite:///hosting.db").replace("postgres://", "postgresql://", 1)
app.config["SQLALCHEMY_TRACK_MODIFICATIONS"] = False
app.config["UPLOAD_FOLDER"] = os.environ.get("UPLOAD_FOLDER", "users")
...
def init_app():
    with app.app_context():
        ...
        print("GitHub token configured:", "YES" if GITHUB_TOKEN else "NO")
        print("="*40)
